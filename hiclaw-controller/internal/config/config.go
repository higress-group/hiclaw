...
func LoadConfig() *Config {
	...
	// Auto-generate AppService tokens when AS mode is enabled but tokens
	// are not explicitly provided. This allows zero-config startup while
	// still supporting explicit token injection for production use.
	if cfg.MatrixAppServiceEnabled {
		if cfg.MatrixAppServiceASToken == "" {
			cfg.MatrixAppServiceASToken = generateRandomHex(32)
			cfg.MatrixAppServiceASTokenAutoGenerated = true
		}
		if cfg.MatrixAppServiceHSToken == "" {
			cfg.MatrixAppServiceHSToken = generateRandomHex(32)
			cfg.MatrixAppServiceHSTokenAutoGenerated = true
		}
	}
